docker_build('sweez/auth:latest', './auth', live_update=[
  sync('./auth/src', '/app/src')
])
k8s_yaml('./auth/k8s/deploy.yml')

docker_build('sweez/client:latest', './client', live_update=[
  sync('./client/pages', '/app/pages')
])
k8s_yaml('./client/k8s/deploy.yml')

docker_build('sweez/tickets:latest', './tickets', live_update=[
